...
- Faz 4 Tamamlandı (20.07.2026): `RentalRepository` domain katmanına taşındı. `domain/model/Rental.kt`, `ActiveRental.kt`, `RentalVehicleSummary.kt`, `RentalPhotos.kt`, `PaymentRequest.kt`, `VehicleLocation.kt` eklendi; `RentalResponseDto`/`FinishRentalResponseDto` aynı alan setine sahip olduğundan ikisi de `Rental`'a eşleniyor. `payRental` artık `PayRentalDto` yerine `PaymentRequest` (sealed) parametresi alıyor ve `Result<Unit>` dönüyor — `PayRentalResponseDto`'nun hiçbir alanı hiçbir ekranda okunmadığından domain modeli eklenmedi. `getStats()` kaldırıldı: hiçbir tüketicisi yoktu, `RentalStatsResponseDto`'nun 5 alanından hiçbiri kullanılmıyordu — domain modeli icat etmek yerine ölü kod olarak silindi. `getPhotosState`/`uploadPhoto` ortak `RentalPhotos` modelini kullanıyor. Tüketiciler (`ReservationViewModel`, `ActiveRentalViewModel`, `VehicleConditionViewModel`, `TripSummaryViewModel`, `PaymentCheckoutViewModel`, `HomeViewModel`, `HistoryRepositoryImpl`) güncellendi. Bu, backlog'un son parçasıydı; artık tüm repository'ler `domain/model/` döndürüyor.
