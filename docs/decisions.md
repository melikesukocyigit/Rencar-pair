...
- Yakınlık Sayacı (Konumuma Git): Kullanıcı, harita Türkiye geneli görünümdeyken "Yakınında N araç"ın gerçek toplamı (146+) göstermesini, ama sağdaki "konumuma git" butonuna basınca gerçekten yakındaki araç sayısına geçmesini istedi. Backend'de konum bazlı filtre olmadığından bu tamamen istemci tarafında, `HomeUiState.nearbyVehicleCount` ile çözüldü: `isFocusedOnUserLocation` (yalnızca "konumuma git" tıklanınca `true` olur) `true` iken sayaç, `NEARBY_RADIUS_METERS = 5000` (kullanıcıyla görüşülüp belirlenen 5 km eşiği) içindeki araçları Haversine mesafesiyle sayıyor; aksi halde (harita geneli görünümde) tüm eşleşen araçların sayısını göstermeye devam ediyor.
